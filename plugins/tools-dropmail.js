  ...
  if (lister.includes(feature)) {
    if (feature == 'crear') {
      try {
        ...
        conn.dropmail[id] = [
          await m.reply('*EMAIL:*\n' + eml[0] + '\n\n' + '*ID:*\n' + eml[1] + '\n\n*expira:*\n' + msToTime(timeDiff) + '\n\nEjemplo *' + usedPrefix + command + ' mensajes* Para comprobar la bandeja de entrada'),
          eml[0],
          eml[1],
          eml[2],
        ];
...
async function get_mails(id_) {
  const link = `${drop}/api/graphql/web-test-wgq6m5i?query=query%20(%24id%3A%20ID!)%20%7Bsession(id%3A%24id)%20%7B%20addresses%20%7Baddress%7D%2C%20mails%7BrawSize%2C%20fromAddr%2C%20toAddr%2C%20downloadUrl%2C%20text%2C%20headerSubject%7D%7D%20%7D&variables=%7B%22id%22%3A%22${id_}%22%7D`;
