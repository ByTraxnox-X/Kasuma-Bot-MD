...
this.pptpvp = this.pptpvp ? this.pptpvp : {}
if (db.data.users[m.sender].pptpvp < 0) db.data.users[m.sender].pptpvp = 0
let room = Object.values(this.pptpvp).find(room => room.id && room.status && [room.p, room.p2].includes(m.sender))
if (room) {
let win = ''
let tie = false
...
let textnull = `@${(room.pilih ? room.p2 : room.p).split`@`[0]} No eligistes ninguno opción, termino el pvpsuerte`
