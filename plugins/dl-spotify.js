...
const handler = async (m, { conn, text }) => {
 if (!text) throw `Ingrese el nombre de la canción.`;
  try {
    ...
    const res = await fetch(global.API('https://api-sebastian.zipponodes.xyz', `/api/spotifysearch?text=${encodedText}`));
    const data = await res.json();
    const linkDL = data.spty.resultado[0].link;
    const musics = await fetch(global.API('https://api-sebastian.zipponodes.xyz', `/api/spotifydl?text=${linkDL}`));
    const music = await conn.getFile(musics.url);
    const infos = await fetch(global.API('https://api-sebastian.zipponodes.xyz', `/api/spotifyinfo?text=${encodedText}`));
