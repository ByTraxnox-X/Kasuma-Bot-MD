let handler = async (m, { conn, text, usedPrefix, command, args }) => {
  ...
  if (command == 'apocalypto') {
    ...
    //opcion de "esperar"
    if (args[0] == "quedarse") {
      m.reply(`Apesar de las advertencias que hizo el estado decides quedarse, el virus se apodero de el mundo. Todo es un caos, ya no hay recursos, y el estado anuncia un virus mortal de propagacion extrema. Tienes solo una opcion y es irte.\n\n${usedPrefix + command} irse`)
    }

    //opcion de "esperar y empezar apocalypto"
    if (args[0] == "irse") {
    m.reply(`Ante la inminente propagacion del virus, tomas la dificil decision de abandonar tu hogar en busca de un refugio seguro. Empacas lo esencial y emprendes un viaje incierto hacia lo desconocido. El mundo esta en caos, pero tu determinacion te impulsa a buscar una oportunidad para sobrevivir. *Tienes dos opciones, hay 2 ciudades, en Crestview hay personas con el virus, pero mas recursos para sobrevivir. En Rivertown no hay casos del virus, pero es una ciudad lejana y por lo tanto no tiene muchos recursos, tu decides*\n\n${usedPrefix}apocalypto Crestview\n${usedPrefix}apocalypto Rivertown`)
    }
    ...
    if (args[0] == "explorar") {
      m.reply(`La decision de explorar ha tomado un rumbo de supervivencia, haciendote estar mas lejos de las ciudades para evitar el virus. Explorando has llegado a un bosque extenso, y has encontrado un gato dentro del bosque y decides`)
  }
