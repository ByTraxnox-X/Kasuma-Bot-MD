let handler = async (m, { conn, text, usedPrefix, command, args }) => {
  ...
  if (command == 'apocalypto') {
    ...
    //opcion de "esperar"
    if (args[0] == "quedarse") {
      m.reply(`Apesar de las advertencias que hizo el estado decides quedarse, el virus se apodero de el mundo. Todo es un caos, ya no hay recursos, y el estado anuncia un virus mortal de propagacion extrema.\n*Tienes solo una opcion y es irte.*\n\n${usedPrefix + command} irse`)
    }
